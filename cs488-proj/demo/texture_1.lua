...
bn:add_child(cube_3)
cube_3:set_material(clock_texture)
...
sphere_1:set_material(texture1)
...
sphere_3:set_material(clock_texture)
...
bn:add_child(cyl_3)
cyl_3:set_material(clock_texture)
...
gr.render(scene,
      'texture_1.png', 600, 600,
      {0, 0, 10}, {0, 0, -1}, {0, 1, 0}, 50,
      {0.4, 0.4, 0.4}, {
        gr.light({0, 1.75, 10}, {0.7, 0.7, 0.7}, {1, 0, 0}, 500000, 64, 1, 'square', 1.5) -- pos, colour, attenuation, num photons, power of initial photons, light type ('square','round','point'), power bias, size(if non point light).
