...
bool isZero(double n, double tol)
{
    return (n < tol && n > -tol);
}
...
bool solve2x2linearSystems(double a1, double b1, double c1, double a2, double b2, double c2, Point2D &coords) // a1x+b1y = c1, a2x+b2y = c2
{
    // solve by Cramer's rule
    double determinant = a1 * b2 - b1 * a2;
    if (determinant > EPSILON || determinant < -EPSILON) {
        coords[0] = (c1 * b2 - b1 * c2) / determinant;
        coords[1] = (a1 * c2 - c1 * a2) / determinant;
        return true;
    }
    return false;
}

bool solve1x1linearSystems(double a, double b, double c, double &t) // at + b = c
{
    if (isZero(a)) {
        return false;
    }
...
bool solve3x1linearSystems(const Vector3D &A, const Vector3D &B, const Vector3D &C, double &t)
{
    double a1 = A[0], b1 = B[0], c1 = C[0];
    double a2 = A[1], b2 = B[1], c2 = C[1];
    double a3 = A[2], b3 = B[2], c3 = C[2];
    double b12 = true, b23 = true, b31 = true;
    if (solve1x1linearSystems(a1, b1, c1, t)) {
        if (!isZero(a2 * t + b2 - c2, 0.1) || !isZero(a3 * t + b3 - c3, 0.1)) {
//            std::cerr << "1" << std::endl;
            b23 = false;
        }
    } else {
    ...
    if (solve1x1linearSystems(a2, b2, c2, t)) {
        if (!isZero(a1 * t + b1 - c1, 0.1) || !isZero(a3 * t + b3 - c3, 0.1)) {
//            std::cerr << "2" << std::endl;
            b31 = false;
        }
    } else {
    ...
    if (solve1x1linearSystems(a3, b3, c3, t)) {
        if (!isZero(a2 * t + b2 - c2, 0.1) || !isZero(a1 * t + b1 - c1, 0.1)) {
//            std::cerr << "3" << std::endl;
            b12 = false;
        }
    } else {
...
bool solve3x2linearSystems(const Vector3D &A, const Vector3D &B, const Vector3D &C, Point2D &coords)
{
    double a1 = A[0], b1 = B[0], c1 = C[0];
    double a2 = A[1], b2 = B[1], c2 = C[1];
    double a3 = A[2], b3 = B[2], c3 = C[2];
    bool b12 = true, b23 = true, b31 = true;
    if (solve2x2linearSystems(a1, b1, c1, a2, b2, c2, coords)) {
        // plug coords into the third equation.

        if (!isZero(a3 * coords[0] + b3 * coords[1] - c3)) {
            return false;
        }
    } else {
        b12 = false;
    ...
    if (solve2x2linearSystems(a2, b2, c2, a3, b3, c3, coords)) {
        // plug coords into the third equation.
        if (!isZero(a1 * coords[0] + b1 * coords[1] - c1)) {
            return false;
        }
    } else {
        b23 = false;
    ...
    if (solve2x2linearSystems(a3, b3, c3, a1, b1, c1, coords)) {
        // plug coords into the third equation.
        if (!isZero(a2 * coords[0] + b2 * coords[1] - c2)) {
            return false;
        }
    } else {
        b31 = false;
