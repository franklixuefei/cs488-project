...
bool isZero(double n, double tol=EPSILON);
...
public:
  ...
  double min_minus(const Point3D& other) const
  {
    double v0 = v_[0] - other.v_[0];
    double v1 = v_[1] - other.v_[1];
    double v2 = v_[2] - other.v_[2];
    return (double)(std::min((double)(std::min(v0, v1)), v2));
  }
    const double *getRaw()
    {
        return v_;
