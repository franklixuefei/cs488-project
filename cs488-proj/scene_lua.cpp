...
int gr_corbox_cmd(lua_State *L)
{
    ...
    data->node = new GeometryNode("Cornell Box", new CornellBox(width, height));
...
int gr_torus_cmd(lua_State *L)
{
    ...
    double inner_radius = luaL_checknumber(L, 2);
    double outer_radius = luaL_checknumber(L, 3);
    
    data->node = new GeometryNode(name, new Torus(inner_radius, outer_radius));
